...
def prepare_predictions(data, max):
    df = pd.DataFrame()
    days = []
    ...
    df['holiday'] = df['day'].apply(lambda x: calculate_holiday(x))
    for i in range(0, max):
...
data['holiday'] = data['day'].apply(lambda x: calculate_holiday(x))
...
to_predict_df = prepare_predictions(final_data, max_days_before)
